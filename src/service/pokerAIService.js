...
const getVariants = async numBots => {
    return await axios
        .get(`${FLASK_ENDPOINT}variants/${numBots}`)
        .then(response => {
...
const getAIBuyIn = async (variant, game, botUsername) => {
    return await axios
        .post(`${FLASK_ENDPOINT}buyIn/${variant}`, { botId: botUsername, game })
        .then(response => {
            return response.data
        })
        .catch(err => {
...
const getAIMove = async (variant, game, hand, playOptions, botUsername) => {
    return await axios
        .post(`${FLASK_ENDPOINT}bot/${variant}`, { botId: botUsername, game, hand, playOptions })
        .then(response => {
            return response.data
        })
        .catch(err => {
